pub mod logic {
    ...
    pub struct Device<'h> {
        ...
        signal_counter_ins: [Option<signal::state_source::Signal<hardware::CounterInValue>>;
            hardware::COUNTER_INS_COUNT],
    ...
    impl<'h> Device<'h> {
        pub fn new(hardware_device: &'h hardware::Device) -> Self {
            ...
            Self {
                ...
                signal_ds18x20s: block_functions_reversed
                    ...
                    .into_inner()
                    .unwrap(),
                signal_counter_ins: block_functions_reversed
                    .counter_in_mask
                    .iter()
                    .copied()
                    .map(|counter_in_enabled| {
                        if counter_in_enabled {
                            Some(
                                signal::state_source::Signal::<hardware::CounterInValue>::new(None),
                            )
                        } else {
                            None
                        }
                    })
                    .collect::<ArrayVec<_, { hardware::COUNTER_INS_COUNT }>>()
                    .into_inner()
                    .unwrap(),
        ...
        fn properties_ins_changed(&self) {
            ...
            if let Some(counter_ins) = self.properties_remote.counter_ins.take_pending() {
                if let Some(counter_ins) = counter_ins {
                    self.signal_counter_ins
                        .iter()
                        .zip_eq(counter_ins)
                        .filter_map(|(signal_counter_in, counter_in_value)| {
                            signal_counter_in
                                .as_ref()
                                .map(|signal_counter_in| (signal_counter_in, counter_in_value))
                        })
                        .for_each(|(signal_ds18x20, ds18x20_value)| {
                            signals_sources_changed |= signal_ds18x20.set_one(Some(ds18x20_value));
                        });
                    ...
                    self.signal_counter_ins
                        .iter()
                        .filter_map(|signal_counter_in| signal_counter_in.as_ref())
                        .for_each(|signal_counter_in| {
                            signals_sources_changed |= signal_counter_in.set_one(None);
                        });
                }
    ...
    impl signals::Device for Device<'_> {
        ...
        fn by_identifier(&self) -> signals::ByIdentifier<'_, Self::Identifier> {
            chain!(
                ...
                self.signal_counter_ins.iter().enumerate().filter_map(
                    |(counter_in_index, signal_counter_in)| {
                        signal_counter_in.as_ref().map(|signal_counter_in| {
                            (
                                SignalIdentifier::CounterIn(counter_in_index),
                                signal_counter_in as &dyn signal::Base,
                            )
                        })
                    },
                ),
    ...
    enum GuiSummaryBlock1Value {
        ...
        CounterIn(Option<hardware::CounterInValue>),
    }
    #[derive(Debug, Serialize)]
    #[serde(tag = "function", content = "value")]
    ...
    impl devices::gui_summary::Device for Device<'_> {
        ...
        fn value(&self) -> Self::Value {
            ...
            let counter_ins = self.properties_remote.counter_ins.peek_last();
            ...
            let block_1_values = block_functions
                ...
                .map(|(index, block_1_function)| match block_1_function {
                    ...
                    hardware::Block1Function::CounterIn => GuiSummaryBlock1Value::CounterIn(
                        counter_ins.as_ref().map(|counter_in| counter_in[index]),
                    ),
                })
            ...
            let block_2_values = block_functions
                ...
                .map(|(index, block_2_function)| match block_2_function {
                    ...
                    hardware::Block2Function::CounterIn => GuiSummaryBlock2Value::CounterIn(
                        counter_ins
                            .as_ref()
                            .map(|counter_ins| counter_ins[hardware::BLOCK_1_SIZE + index]),
                    ),
                })
...
pub mod hardware {
    use super::super::super::{
        ...
        hardware::{
            datatypes::ds18x20::SensorState as Ds18x20SensorState,
    ...
    use crate::{
        datatypes::{color_rgb_boolean::ColorRgbBoolean, frequency::Frequency, voltage::Voltage},
        util::{
            async_ext::stream_take_until_exhausted::StreamTakeUntilExhaustedExt,
    ...
    pub type CounterInValue = Frequency;
    type CounterInValueRaw = u16;
    pub const COUNTER_INS_COUNT: usize = BLOCK_1_SIZE + BLOCK_2_SIZE;
    pub type CounterInValues = [CounterInValue; COUNTER_INS_COUNT];
    type CounterInValuesRaw = [CounterInValueRaw; COUNTER_INS_COUNT];
    ...
    pub struct BlockFunctionsReversed {
        ...
        pub counter_in_mask: [bool; COUNTER_INS_COUNT],
        pub counter_in_any: bool,
    }
    impl BlockFunctionsReversed {
        pub fn new(block_functions: &BlockFunctions) -> Self {
            ...
            let counter_in_mask = chain!(
                block_functions
                    .block_1_functions
                    .iter()
                    .map(|block_1_function| *block_1_function == Block1Function::CounterIn),
                block_functions
                    .block_2_functions
                    .iter()
                    .map(|block_2_function| *block_2_function == Block2Function::CounterIn),
            )
            .collect::<ArrayVec<_, { COUNTER_INS_COUNT }>>()
            .into_inner()
            .unwrap();
            let counter_in_any = counter_in_mask
                .iter()
                .any(|counter_in_enabled| *counter_in_enabled);
    ...
    pub struct PropertiesRemote<'p> {
        ...
        pub counter_ins: properties::state_in::Remote<'p, CounterInValues>,
    }
    ...
    pub struct Properties {
        ...
        counter_ins: properties::state_in::Property<CounterInValues>,
    }
    impl Properties {
        pub fn new() -> Self {
            Self {
                ...
                counter_ins: properties::state_in::Property::<CounterInValues>::new(),
            }
        }
        ...
        pub fn device_reset(&self) -> bool {
            ...
            properties_ins_changed |= self.counter_ins.device_reset();
        ...
        pub fn remote(&self) -> PropertiesRemote<'_> {
            PropertiesRemote {
                ...
                counter_ins: self.counter_ins.user_remote(),
            }
        }
    }
    ...
    pub struct Device {
        ...
        counter_ins_read_last: AtomicRefCell<Option<Instant>>,
        properties: Properties,

        poll_waker: async_waker::mpsc::Signal,
    }
    impl Device {
        const COUNTER_IN_READ_INTERVAL_MIN: Duration = Duration::from_secs(1);
        ...
        pub fn new(configuration: Configuration) -> Self {
            ...
            Self {
                ...
                counter_ins_read_last: AtomicRefCell::new(None),
    ...
    impl runner::BusDevice for Device {
        ...
        fn poll_delay(&self) -> Option<Duration> {
            ...
            if false
                ...
                || self.block_functions_reversed.counter_in_any
            {
        ...
        async fn poll(
            ...
            // stage 1 is always executed to check the device
            ...
            let BusResponse {
                ...
                counter_ins: stage_1_response_counter_ins,
            } = stage_1_response;
            ...
            ensure!(stage_1_response_counter_ins.is_none());
            ...
            #[derive(Debug)]
            enum CounterInsReadPeriod {
                Enough(Duration),
                NotYet,
                Unknown,
            }
            let counter_ins_read_period = match *self.counter_ins_read_last.borrow() {
                Some(counter_ins_read_last) => {
                    let counter_ins_read_period = now - counter_ins_read_last;
                    if counter_ins_read_period >= Self::COUNTER_IN_READ_INTERVAL_MIN {
                        CounterInsReadPeriod::Enough(counter_ins_read_period)
                    } else {
                        CounterInsReadPeriod::NotYet
                    }
                }
                None => CounterInsReadPeriod::Unknown,
            };
            ...
            let stage_2_request = BusRequest {
                ...
                analog_ins_request: self.block_functions_reversed.analog_in_any
                    && (false
                        ...
                        || stage_1_response_poll.as_ref().unwrap().analog_ins),
                digital_ins_request: false,
                digital_outs: None,
                ds18x20s_request: self.block_functions_reversed.ds18x20_any
                    && (false
                        || self.properties.ds18x20s.device_must_read()
                        || stage_1_response_poll.as_ref().unwrap().ds18x20s),
                counter_ins_request: true
                    // there must be at least one counter in expected
                    && self.block_functions_reversed.counter_in_any
                    // device must report there is something to read
                    && stage_1_response_poll.as_ref().unwrap().counter_ins
                    // minimal period must have had elapsed (or isn't set)
                    && matches!(
                        counter_ins_read_period,
                        CounterInsReadPeriod::Unknown | CounterInsReadPeriod::Enough(_)
                    ),
            };

            // if stage_2_request is nop, we fake the response to be empty
            let stage_2_response = if stage_2_request != BusRequest::nop() {
            ...
            let BusResponse {
                ...
                counter_ins: stage_2_response_counter_ins,
            } = stage_2_response;
            ...
            let stage_2_response_counter_ins = match (
                stage_2_request.counter_ins_request,
                stage_2_response_counter_ins,
            ) {
                (false, None) => None,
                (true, Some(stage_2_response_counter_ins)) => Some(stage_2_response_counter_ins),
                _ => bail!("counter_ins mismatch"),
            };
            ...
            if self.block_functions_reversed.counter_in_any {
                match (
                    stage_1_response_poll.as_ref().unwrap().counter_ins,
                    counter_ins_read_period,
                ) {
                    (true, CounterInsReadPeriod::Enough(read_period)) => {
                        // enough time has elapsed and we have non-zero values waiting, use them
                        let values = stage_2_response_counter_ins
                            .unwrap()
                            .values
                            .into_iter()
                            .map(|value| {
                                let hertz = (value as f64)
                                    / 2.0 // NOTE: /2 because each slope is calculated
                                    / (read_period.as_secs_f64());
                                Frequency::from_hertz(hertz).unwrap()
                            })
                            .collect::<ArrayVec<_, COUNTER_INS_COUNT>>()
                            .into_inner()
                            .unwrap();
                    ...
                    (true, CounterInsReadPeriod::Unknown) => {
                        // this is first read, we don't know for how long it was counting
                        // discard the value, but set the timer, next time we will have it
                        assert!(stage_2_response_counter_ins.is_some());

                        self.counter_ins_read_last.borrow_mut().replace(now);
                    }
                    (false, CounterInsReadPeriod::Enough(_) | CounterInsReadPeriod::Unknown) => {
                        // either enough time has elapsed or we don't know how much time has
                        // elapsed, but the poll flag is false, so input count is zero.
                        assert!(stage_2_response_counter_ins.is_none());

                        let values = iter::repeat_n(Frequency::zero(), COUNTER_INS_COUNT)
                            .collect::<ArrayVec<_, COUNTER_INS_COUNT>>()
                            .into_inner()
                            .unwrap();
                    ...
                    (true, CounterInsReadPeriod::NotYet)
                    | (false, CounterInsReadPeriod::NotYet) => {
                        // not enough time has elapsed, so we don't care if the value is waiting or
                        // not
                        assert!(stage_2_response_counter_ins.is_none());
                    }
                }
            ...
            // common for stages 1 and 2
            if properties_ins_changed {
        ...
        fn reset(&self) {
            *self.counter_ins_read_last.borrow_mut() = None;
    ...
    impl BusRequest {
        ...
        pub fn serialize(
            ...
            if self.counter_ins_request {
                serializer.push_byte(b'N');
            }
        }
    }
    #[cfg(test)]
    mod tests_bus_request {
        use super::{
            super::super::super::super::houseblocks_v1::common::Payload, Block1Function,
            Block2Function, Block3Function, Block4Function, BlockFunctions, BusRequest,
            BusRequestConfiguration, BusRequestDigitalOut, BusRequestStatusLed, StatusLedValue,
        };
        ...
        fn everything() {
            ...
            let payload_expected = Payload::new(Box::from(*b"CAIONIIOTL111PAIO07FFTN")).unwrap();
    ...
    impl BusResponsePoll {
        pub fn parse(parser: &mut Parser) -> Result<Self, Error> {
            ...
            let counter_ins = parser.expect_bool().context("counter_ins")?;
            Ok(Self {
                analog_ins,
                ds18x20s,
                counter_ins,
    ...
    #[derive(PartialEq, Eq, Debug)]
    struct BusResponseCounterIns {
        pub values: CounterInValuesRaw,
    }
    impl BusResponseCounterIns {
        pub fn parse(parser: &mut Parser) -> Result<Self, Error> {
            let values = (0..COUNTER_INS_COUNT)
                .map(|_counter_in_index| -> Result<_, Error> {
                    let value = parser.expect_u16().context("expect_u16")?;
                    Ok(value)
                })
                .collect::<Result<ArrayVec<_, { COUNTER_INS_COUNT }>, _>>()
                .context("collect")?
                .into_inner()
                .unwrap();
            Ok(Self { values })
    ...
    struct BusResponse {
        ...
        pub counter_ins: Option<BusResponseCounterIns>,
    }
    impl BusResponse {
        pub const fn nop() -> Self {
            Self {
        ...
        pub fn parse(parser: &mut Parser) -> Result<Self, Error> {
            ...
            let mut counter_ins = Option::<BusResponseCounterIns>::None;
            ...
            while let Some(opcode) = parser.get_byte() {
                match opcode {
                    ...
                    b'N' => {
                        let value = BusResponseCounterIns::parse(parser).context("counter_ins")?;
                        ensure!(
                            counter_ins.replace(value).is_none(),
                            "duplicated counter_ins"
                        );
                    }
    ...
    mod tests_bus_response {
        use super::{
            ...
            BusResponse, BusResponseAnalogIns, BusResponseCounterIns, BusResponseDigitalIns,
            BusResponseDs18x20s, BusResponsePoll,
        };
        use crate::datatypes::{
        ...
        fn full() {
            let payload = Payload::new(Box::from(
                *b"P111A000001000200040002000400IA4T000087D09191CC90N0123456789ABCDEFFEDCBA9876543210",
            ))
            .unwrap();
            let bus_response = BusResponse::from_payload(&payload).unwrap();
            ...
            let bus_response_expected = BusResponse {
                ...
                counter_ins: Some(BusResponseCounterIns {
                    values: [
