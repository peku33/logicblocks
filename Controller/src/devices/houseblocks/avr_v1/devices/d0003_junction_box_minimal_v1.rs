...
pub mod hardware {
            ...
            #[cfg(feature = "ci-devices-houseblocks-avr_v1-firmware")]
            {
                static FIRMWARE: std::sync::LazyLock<Firmware> = std::sync::LazyLock::new(|| {
                    let content = include_bytes!(concat!(
                        ...
                        "/JunctionBox_Minimal_v1_Application.bin"
                    ));
    ...
    impl runner::BusDevice for Device {
        ...
        async fn poll(
            ...
            if stage_2_request == BusRequest::nop() {
                return Ok(());
            }
    ...
    mod tests_bus_request {
        use super::{
            super::super::super::super::houseblocks_v1::common::Payload, BusRequest,
            BusRequestBuzzer, BusRequestLeds,
        };
