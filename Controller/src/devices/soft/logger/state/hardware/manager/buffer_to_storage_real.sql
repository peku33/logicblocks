WITH
    `t1` AS (
        SELECT
            `buffer_real`.ROWID AS `rowid_`
        FROM
            `buffer_real`
        JOIN
            `sinks_ext_real` USING(`sink_id`)
        WHERE
...
INSERT INTO
    `storage_real`
    (`sink_id`, `timestamp_group_start`, `value_last_timestamp`, `value_last_value`, `weight`, `sum`, `min`, `max`)
SELECT
    `sink_id`, `timestamp_group_start`, `value_last_timestamp`, `value_last_value`, `weight`, `sum`, `min`, `max`
FROM
    `t3`
...
DO UPDATE SET
    ...
    `min` = COALESCE(MIN(`min`, EXCLUDED.`min`), `min`, EXCLUDED.`min`),
    `max` = COALESCE(MAX(`max`, EXCLUDED.`max`), `max`, EXCLUDED.`max`)
;
...
FROM
    `t2`
WHERE
    `sinks_ext_real`.`sink_id` = `t2`.`sink_id`
;
