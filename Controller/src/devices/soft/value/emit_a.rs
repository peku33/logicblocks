...
where
    V: Value + Clone,
{
    /// value emitted on every trigger
    pub value: V,
}

/// when Event<()> hits trigger target
/// configured fixed value is emitted as Event<V> from output source
#[derive(Debug)]
pub struct Device<V>
where
    V: Value + Clone,
    ...
    V: Value + Clone,
{
    ...
    fn signals_targets_changed(&self) {
        ...
        let output = self.configuration.value.clone();
    ...
    V: Value + Clone,
{
    fn class(&self) -> Cow<'static, str> {
        Cow::from(format!("soft/value/emit_a<{}>", type_name::<V>()))
    }
