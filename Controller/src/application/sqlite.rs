use super::fs::Fs;
use super::modules::{ModuleFactory, ModuleFactoryTrait, ModuleHandle, ModuleTrait};
...
impl Sqlite {
    pub fn new(fs: ModuleHandle<Fs>) -> Self {
        ...
        let sqlite_async = SqliteAsync::new(sqlite_connection, "SQLite Module".to_owned());
...
impl ModuleTrait for Sqlite {
}
impl ModuleFactoryTrait for Sqlite {
    fn spawn<'mf>(module_factory: &'mf ModuleFactory) -> BoxFuture<'mf, Self> {
        async move {
            let fs = module_factory.get::<Fs>().await;
            Self::new(fs)
        }
        .boxed()
    }
